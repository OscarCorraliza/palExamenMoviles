...
public class TipoActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        listView.setOnItemClickListener((parent, view, position, id) -> {
            Intent intent = new Intent(TipoActivity.this, PokemonsActivity.class);
    ...
    public void getTipos() {
        ...
        apiPokemon.getTipos().enqueue(new Callback<JsonResponse>() {
            @Override
            public void onResponse(Call<JsonResponse> call, Response<JsonResponse> response) {
                if (response != null && response.body() != null) {
                    resources = response.body().results;

                    adapter = new PokemonAdapter(TipoActivity.this, resources);
